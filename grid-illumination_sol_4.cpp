...
public:
    ...
    unordered_map<int,int> rowIlluminated;
    unordered_map<int,int> columnIlluminated;
    unordered_map<int,int> plusDiagIlluminated;
    unordered_map<int,int> minusDiagIlluminated;
    
    unordered_map<pair<int,int>,bool,hash_pair> isLampOn;
    int n;
    Solution(){}
    ...
    void reduceByOneIfPositive(unordered_map<int,int> &hashMap, int index){
        if (hashMap[index]>0){
            hashMap[index]--;
        }
    }
